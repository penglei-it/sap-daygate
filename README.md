...
2. 查看完成率、连续天数、软提醒  
3. 给今日留陪伴留言（学习者可见）  
4. 可「复制周报」或「下载周报.txt」（近 7 日纯文本，仅本机）  
5. 不可代打卡；可用 PIN 退出监护视图  

## 安装到主屏幕（PWA）

可用浏览器「添加到主屏幕」，把日验装成离线壳应用。GitHub Pages 站点 base 为 `/sap-daygate/`，manifest 的 `start_url` / `scope` 使用相对路径以跟随部署 base。
